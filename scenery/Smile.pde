class Smile implements GrafOBJ{
  ...
  void display(){
    ...
    endShape();
    line(shapeSize*0.4, -shapeSize*0.1+shapeMovementVelocity, shapeSize*0.4,-shapeSize*0.6+shapeMovementVelocity);
    line(-shapeSize*0.4, -shapeSize*0.1+shapeMovementVelocity, -shapeSize*0.4,-shapeSize*0.6+shapeMovementVelocity);
    
    beginShape();
    for (float angle=PI/4; angle<HALF_PI+QUARTER_PI; angle+=0.01){
      float x = shapeSize *cos(angle);
      float y = shapeSize*sin(angle);
      vertex(x,y-shapeSize*0.4+shapeMovementVelocity);
    }
    endShape();
    popMatrix();
  }
